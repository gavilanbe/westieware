// ============================================================================
//  ANAHÍ'S BOSS — ¡PREPARA EL PERRO!  (the user's own idea, from his parody video)
//  Lady Di arrives a mess four minutes before the 12:00 dog show. The full
//  Westie BLVRD service, phase by phase:
...
defMG({
  id: 'prepara', stage: 'anahi', boss: true, name: '¡Prepara el perro!', cmd: '¡PREPARA EL PERRO!', how: 'El servicio completo: cepilla, champú, frota, aclara, seca, corta… y cobra', mech: 'mix', beats: 16,
  song: () => PREP_SONG,
  init(g) {
  ...
  drawDog(g, c, room) {
    ...
    if (ph === 0) {
      ...
      if (ph >= 6) { c.drawImage(prepDog('clean', 1, 'proud'), rd(dx), rd(dy + bob)); prepLadyBow(c, dx + 58, dy + bob + 20, 1); }
  ...
  drawTill(g, c) {
    ...
    prepOwner(c, 62, 104, paid ? 'happy' : 'wait', t);
    // Lady Di, gleaming, sits on the counter next to her owner
    drawS(c, prepDog('clean', .56, 'proud'), 26, 126, { ax: .5, ay: 1 }); prepLadyBow(c, 30, 76, .56);
  ...
  top(g, c) {
    ...
    tiny(c, 'CLIENTA: LADY DI', 56, 98, INK, { align: 'c' });
    if (st === 'clean') prepLadyBow(c, 60, 117, .6);
    // patience bar
...
// Keiko cheering in the HUD corner, paws up
function prepKeikoCheer(g, x, y, t, ex) {
  const bob = Math.abs(Math.sin(t * 5)) * 3, wave = Math.sin(t * 10) * 3;
  for (const sx of [-1, 1]) { const px0 = x + sx * 24, py0 = y - 44 - bob + (sx < 0 ? wave : -wave); disc(g, px0, py0, 6, INK); disc(g, px0, py0, 5, '#ffffff'); disc(g, px0 - 1, py0 - 1, 2.5, '#f6f1ee'); for (const d of [-2, 0, 2]) px(g, px0 + d, py0 + 3, '#c8c6d3'); }
  const head = keikoHead(ex === 'sad' ? 'sad' : ex === 'wow' ? 'wow' : fl(t * 1.5) % 4 === 0 ? 'wink' : 'happy');
  drawS(g, head, x, y - 30 - bob, { ax: .5, ay: .5 });
}
// Lady Di's pink show bow (x, y = its knot)
function prepLadyBow(g, x, y, k = 1) {
  const s = v => v * k, bx = rd(x), by = rd(y);
  polyPx(g, [[bx, by], [bx - s(11), by - s(7)], [bx - s(11), by + s(7)]], INK); polyPx(g, [[bx, by], [bx + s(11), by - s(7)], [bx + s(11), by + s(7)]], INK);
  polyPx(g, [[bx, by], [bx - s(10), by - s(6)], [bx - s(10), by + s(6)]], '#ff5d9e'); polyPx(g, [[bx, by], [bx + s(10), by - s(6)], [bx + s(10), by + s(6)]], '#ff5d9e');
